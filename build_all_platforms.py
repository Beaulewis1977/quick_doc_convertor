...
def main():
    ...
    parser.add_argument(
        "--version", default="2.0.0", help="Package version (default: 2.0.0)"
    )
    parser.add_argument("--icon", type=Path, help="Path to application icon")
    parser.add_argument(
        "--install-deps",
        action="store_true",
...
def build_windows_packages(
    script_path, output_dir, version, icon_path, verbose
):
    ...
    return build_all_windows_packages(
        script_path, output_dir, version, icon_path
    )
...
def build_macos_packages(script_path, output_dir, version, icon_path, verbose):
    ...
    return build_all_macos_packages(
        script_path, output_dir, version, icon_path
    )
