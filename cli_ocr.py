...
try:
    from convert_to_markdown import convert_pdf_to_markdown, convert_to_markdown
...
Examples:
  ...
  %(prog)s --check-ocr                              # Check OCR availability
        ...
        # OCR options
        parser.add_argument(
            "--ocr", action="store_true", help="Enable OCR for images and PDFs"
        )
        parser.add_argument(
            "--ocr-lang", default="eng", help="OCR language (default: eng)"
        )
        parser.add_argument(
            "--check-ocr",
            action="store_true",
        ...
        parser.add_argument(
            "--version", action="version", version="OCR Document Converter 2.1"
        )
    ...
    def run_setup(self):
        ...
        try:
            ...
            print(
                "2. Install Python packages: pip install -r requirements_updated.txt"
            )
            return False

    def convert_file_with_ocr(
        self,
        input_path: str,
        output_path: str,
        ...
        try:
            ...
            if ocr_enabled and file_ext in [
                ".png",
                ".jpg",
                ".jpeg",
                ".tiff",
    ...
    def run_conversion(self, args):
        ...
        if len(files) == 1 and not Path(args.output).is_dir():
            ...
            success = self.convert_file_with_ocr(
                str(files[0]), str(output_file), args.ocr, args.ocr_lang
            )
            ...
            for i, file in enumerate(files, 1):
                ...
                success = self.convert_file_with_ocr(
                    str(file), str(output_file), args.ocr, args.ocr_lang
                )
