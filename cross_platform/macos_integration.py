    ...
    try:
        ...
        try:
            ...
            raise MacOSIntegrationError(
                "py2app not installed. Install with: pip install py2app"
            )

        # Create setup.py content
        setup_content = create_py2app_setup(
            script_path, app_name, icon_path=icon_path
        )
...
def create_dmg_installer(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.returncode != 0:
                raise MacOSIntegrationError(
                    f"DMG creation failed: {result.stderr}"
                )
...
def setup_macos_integration(
    script_path: Path, output_dir: Path, icon_path: Optional[Path] = None
) -> Dict[str, any]:
    """
