...
def demo_tray():
    ...
    if missing:
        print(f"❌ Missing dependencies: {', '.join(missing)}")
        response = (
            input("Install missing dependencies? (y/n): ").lower().strip()
        )
    ...
    try:
        ...
        print(
            "💡 Look for the Quick Document Convertor icon in your system tray"
        )
        ...
        print(
            "Make sure enhanced_system_tray.py exists in the current directory"
        )
