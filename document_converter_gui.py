...
class DocumentConverterGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Label(
            main_frame, text="Source Folder:", font=("Arial", 10, "bold")
        ...
        # Progress bar
        ttk.Label(
            main_frame, text="Progress:", font=("Arial", 10, "bold")
        ...
        # Results text area
        ttk.Label(
            main_frame, text="Results:", font=("Arial", 10, "bold")
        ).grid(row=10, column=0, sticky=tk.W, pady=(15, 5))
        ...
        self.results_text = tk.Text(
            text_frame, height=8, wrap=tk.WORD, font=("Consolas", 9)
        )
    ...
    def setup_drag_drop(self):
        ...
        try:
            ...
            instruction_text = (
                "💡 Tip: Use the Browse buttons to select folders"
            )
            ttk.Label(
                self.root,
                text=instruction_text,
    ...
    def on_drop(self, event):
        ...
        if files:
            ...
            if os.path.isdir(dropped_path):
                ...
                self.log_message(
                    f"📁 File dropped, using parent folder: {parent_dir}"
                )

    def browse_input_folder(self):
        """Browse for input folder"""
        folder = filedialog.askdirectory(
            title="Select folder containing documents to convert"
        )
        if folder:
    ...
    def convert_documents(self):
        ...
        try:
            ...
            if total_files == 0:
                self.log_message(
                    "❌ No convertible files found (looking for .docx, .pdf, .txt)"
                )
            ...
            for i, file_path in enumerate(files_to_convert):
                try:
                    ...
                    if (
                        ...
                        and not self.overwrite_existing.get()
                    ):
                        self.log_message(
                            f"⏭️  Skipped (exists): {file_path.name}"
                        )
                        continue
