...
class QuickConverterTray:
    ...
    def create_icon_image(self) -> Image.Image:
        ...
        draw.polygon(
            [(42, 8), (52, 8), (52, 18), (42, 8)], fill=(135, 206, 235, 255)
        )
        draw.line(
            [(42, 8), (42, 18), (52, 18)], fill=(25, 25, 112, 255), width=1
        )
    ...
    def show_main_app(self, icon=None, item=None):
        ...
        try:
            if self.main_app.exists():
                ...
                self.show_notification(
                    "Quick Document Convertor", "Application started"
                )
            else:
                self.show_error(
                    "Main application not found!",
                    f"Could not find: {self.main_app}",
                )
        except Exception as e:
            self.show_error(
                "Launch Error", f"Failed to start application: {e}"
            )
        ...
        try:
            ...
            if file_path:
                ...
                if cli_script.exists():
                    ...
                    self.show_error(
                        "CLI Not Found", "Command-line interface not available"
                    )
            ...
            self.show_error(
                "Quick Convert Error", f"Failed to convert file: {e}"
            )
    ...
    def show_settings(self, icon=None, item=None):
        ...
        title_label = ttk.Label(
            main_frame, text="System Tray Settings", font=("Arial", 12, "bold")
        )
        ...
        auto_start_cb = ttk.Checkbutton(
            main_frame, text="Start with Windows", variable=auto_start_var
        )
        ...
        notifications_cb = ttk.Checkbutton(
            main_frame, text="Show notifications", variable=notifications_var
        )
        notifications_cb.pack(anchor=tk.W, pady=2)

        # Quick convert checkbox
        quick_convert_var = tk.BooleanVar(
            value=self.config.get("quick_convert_enabled", True)
        )
        quick_convert_cb = ttk.Checkbutton(
            main_frame, text="Enable quick convert", variable=quick_convert_var
        )
        quick_convert_cb.pack(anchor=tk.W, pady=2)

        # Default output format
        ttk.Label(main_frame, text="Default output format:").pack(
            anchor=tk.W, pady=(10, 2)
        )
        format_var = tk.StringVar(
            value=self.config.get("default_output_format", "markdown")
        )
        ...
        ttk.Button(buttons_frame, text="Save", command=save_settings).pack(
            side=tk.RIGHT, padx=(5, 0)
        )
        ttk.Button(buttons_frame, text="Cancel", command=cancel_settings).pack(
            side=tk.RIGHT
        )
        ...
        try:
            ...
            with winreg.OpenKey(
                ...
                if enable:
                    ...
                    if exe_path.exists():
                        winreg.SetValueEx(
                            key, app_name, 0, winreg.REG_SZ, str(exe_path)
                        )
                else:
                    try:
...
© 2024 Beau Lewis. All rights reserved."""
        ...
        # Close button
        ttk.Button(
            content_frame, text="Close", command=about_window.destroy
    ...
    def create_menu(self):
        ...
        if self.config.get("quick_convert_enabled", True):
            menu_items.append(
                pystray.MenuItem(
                    "Quick Convert File...", self.quick_convert_file
                )
            )
    ...
    def run(self):
        ...
        self.icon = pystray.Icon(
            "Quick Document Convertor", icon_image, menu=menu
        )
        ...
        if self.config.get("show_notifications", True):
            ...
            def show_startup_notification():
                ...
                time.sleep(2)
                self.show_notification(
                    "Quick Document Convertor", "System tray loaded"
                )

            threading.Thread(
                target=show_startup_notification, daemon=True
            ).start()
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                if (
                    ...
                    and any("tray_app" in arg for arg in proc.info["cmdline"])
                ):
