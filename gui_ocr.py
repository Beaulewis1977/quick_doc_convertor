...
class OCRGUI:
    ...
    def setup_ui(self):
        ...
        self.ocr_status = ttk.Label(
            main_frame, text="OCR Status: Checking...", foreground="blue"
        )
        ...
        # File entry
        ttk.Label(file_frame, text="Select files or folder:").grid(
        ...
        options_frame = ttk.LabelFrame(
            main_frame, text="Conversion Options", padding="10"
        )
        options_frame.grid(
        ...
        ttk.Checkbutton(
            options_frame,
            text="Enable OCR for images/PDFs",
            variable=self.use_ocr,
        ).grid(row=0, column=0, sticky=tk.W)

        # Language selection
        ttk.Label(options_frame, text="OCR Language:").grid(
        ...
        language_combo = ttk.Combobox(
            ...
            values=["eng", "eng+fra", "eng+spa", "deu", "jpn"],
            width=15,
        )
        ...
        log_frame = ttk.LabelFrame(
            main_frame, text="Conversion Log", padding="10"
        )
        log_frame.grid(
            row=5,
            column=0,
        ...
        self.log_text = tk.Text(
            log_frame, height=10, width=80, yscrollcommand=log_scroll.set
        )
    ...
    def update_ocr_status(self):
        ...
        if self.ocr.is_available():
            self.ocr_status.config(
                text="OCR Status: Ready", foreground="green"
            )
        else:
            self.ocr_status.config(
                text="OCR Status: Tesseract not found - OCR disabled",
                foreground="red",
            )
    ...
    def start_conversion(self):
        ...
        if not path:
            messagebox.showwarning(
                "Warning", "Please select files or folder first!"
            )
            return
    ...
    def convert_files(self, path):
        ...
        try:
            ...
            for i, file_path in enumerate(files_to_process, 1):
                self.log_message(
                    f"Processing file {i}/{total_files}: {os.path.basename(file_path)}"
                )
                ...
                try:
                    ...
                    if output_path:
                        self.log_message(
                            f"✓ Converted: {os.path.basename(file_path)} -> {os.path.basename(output_path)}"
                        )
                    else:
                        self.log_message(
            ...
            if file_ext in [".png", ".jpg", ".jpeg", ".tiff", ".bmp", ".gif"]:
                ...
                if self.use_ocr.get() and self.ocr.is_available():
                    text = self.ocr.extract_text(
                        file_path, self.language_var.get()
                    )
                else:
