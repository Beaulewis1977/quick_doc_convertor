...
class ImageProcessor:
    ...
    def preprocess_image(
        self, image_path: str, options: Dict[str, Any] = None
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            image = self.resize_image(
                image, options.get("max_dimension", 2048)
            )
            image = self.enhance_contrast(
                image, options.get("contrast_factor", 1.5)
            )
            ...
            # Apply thresholding
            image = self.apply_threshold(
                image, options.get("threshold_method", "adaptive")
            )
            ...
            self.logger.error(
                f"Error preprocessing image {image_path}: {str(e)}"
            )
            raise
    ...
    def resize_image(
        self, image: np.ndarray, max_dimension: int = 2048
    ) -> np.ndarray:
    ...
    def enhance_contrast(
        self, image: np.ndarray, factor: float = 1.5
    ) -> np.ndarray:
    ...
    def denoise_image(
        self, image: np.ndarray, apply: bool = True
    ) -> np.ndarray:
    ...
    def apply_threshold(
        self, image: np.ndarray, method: str = "adaptive"
    ) -> np.ndarray:
