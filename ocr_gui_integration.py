...
class OCRGUIIntegration:
    ...
    def create_ocr_widgets(self, parent_frame):
        ...
        ocr_frame = ttk.LabelFrame(
            parent_frame, text="🔍 OCR Mode", padding="10"
        )
        ocr_frame.grid(
        ...
        ttk.Label(self.ocr_settings_frame, text="OCR Engine:").grid(
        ...
        # OCR Output Format
        ttk.Label(self.ocr_settings_frame, text="Output Format:").grid(
        ...
        ttk.Label(self.ocr_settings_frame, text="Language:").grid(
            row=1, column=0, sticky=tk.W, pady=(5, 0), padx=(0, 10)
        )
        ...
        ttk.Label(self.ocr_settings_frame, text="Quality:").grid(
    ...
    def update_parent_gui_for_ocr(self):
        ...
        if hasattr(self.parent_gui, "convert_button"):
            self.parent_gui.convert_button.configure(
    ...
    def restore_parent_gui(self):
        ...
        if hasattr(self.parent_gui, "input_format_combo"):
            ...
            self.parent_gui.input_format_combo.configure(
    ...
    def get_ocr_config(self) -> Dict[str, Any]:
        ...
        quality_map = {
            "fast": {
                ...
                "preprocessing": {"enhance_contrast": False},
            },
            "standard": {
            ...
            "accurate": {
                ...
                "preprocessing": {"enhance_contrast": True, "denoise": True},
            },
        }
    ...
    def process_ocr_files(
        self, file_paths: List[str], output_dir: str, progress_callback=None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.ocr_integration.process_files(
                ...
                max_workers=(
                    self.parent_gui.max_workers.get()
                    if hasattr(self.parent_gui, "max_workers")
                    else 2
                ),
...
class OCRSettingsDialog:
    ...
    def setup_ui(self):
        ...
        engine_frame = ttk.LabelFrame(
            main_frame, text="OCR Engine", padding="10"
        )
        ...
        ttk.Label(
            engine_frame, text=f"Status: {availability['message']}"
        ).grid(row=0, column=0, sticky=tk.W)

        # Cache Management
        cache_frame = ttk.LabelFrame(
            main_frame, text="Cache Management", padding="10"
        )
        cache_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(0, 10))

        ttk.Button(
            cache_frame, text="Clear OCR Cache", command=self.clear_cache
        ).grid(row=0, column=0, sticky=tk.W)

        # Cache Stats
        try:
            cache_stats = (
                self.ocr_gui.ocr_integration.ocr_engine.get_cache_stats()
            )
            if "error" not in cache_stats:
                ttk.Label(
                    cache_frame,
                    text=f"Cache: {cache_stats['file_count']} files, {cache_stats['total_size_mb']} MB",
    ...
    def clear_cache(self):
        ...
        try:
            ...
            if success:
                messagebox.showinfo(
                    "Success", "OCR cache cleared successfully"
                )
            else:
