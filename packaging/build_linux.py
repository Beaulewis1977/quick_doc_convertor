...
"""
            ...
            output_file = (
                output_dir
                / f"quick-document-convertor_{version}_{architecture}.deb"
            )
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            tarball = (
                rpm_build
                / "SOURCES"
                / f"quick-document-convertor-{version}.tar.gz"
            )
            subprocess.run(
                [
                    "tar",
...
def create_appimage(
    app_dir: Path, output_dir: Path, version: str = "2.0.0"
) -> Path:
    """
...
"""
            ...
            try:
                ...
                output_file = (
                    output_dir
                    / f"QuickDocumentConvertor-{version}-x86_64.AppImage"
                )
                ...
                output_file = (
                    output_dir / f"QuickDocumentConvertor-{version}.tar.gz"
                )
                subprocess.run(
                    [
                        "tar",
