...
"""
            ...
            if result.returncode != 0:
                raise PackagingError(
                    f"Failed to create temporary DMG: {result.stderr}"
                )
...
"""
                ...
                if result.returncode != 0:
                    raise PackagingError(
                        f"Failed to compress DMG: {result.stderr}"
                    )
...
def build_all_macos_packages(
    ...
    try:
        ...
        app_bundle = create_pyinstaller_bundle(
    ...
    try:
        ...
        pkg_result = create_pkg_installer(
            app_bundle, pkg_path, version=version
        )
