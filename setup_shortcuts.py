...
def main():
    ...
    try:
        ...
        if integration:
            ...
            app_path = (
                Path(__file__).parent / "universal_document_converter.py"
            )
            icon_path = None
            ...
            if platform == "windows":
                results = integration.setup_windows_integration(
                    app_path, icon_path
                )
                success = results.get("shortcuts", {}).get("desktop", False)
            elif platform == "linux":
                results = integration.setup_linux_integration(
                    app_path, icon_path
                )
            ...
            if success:
                ...
                print(
                    "⚠️  Enhanced integration had some issues, falling back to basic setup"
                )
