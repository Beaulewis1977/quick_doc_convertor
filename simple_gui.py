...
class SimpleConverterGUI:
    ...
    def setup_ui(self):
        ...
        # Input selection
        ttk.Label(
            main_frame, text="Input Files/Folder:", font=("Arial", 10, "bold")
        ...
        ttk.Button(
            input_frame, text="Browse Files", command=self.browse_files
        ).grid(row=0, column=1, padx=(0, 5))
        ttk.Button(
            input_frame, text="Browse Folder", command=self.browse_folder
        ).grid(row=0, column=2)

        # Output selection
        ttk.Label(
            main_frame, text="Output Folder:", font=("Arial", 10, "bold")
        ...
        format_frame = ttk.LabelFrame(
            main_frame, text="Format Options", padding="10"
        )
        format_frame.grid(
        ...
        ttk.Label(format_frame, text="From:").grid(
        ...
        ttk.Label(format_frame, text="To:").grid(
        ...
        self.convert_btn = ttk.Button(
            main_frame, text="Convert Documents", command=self.start_conversion
        )
        ...
        if not CONVERTER_AVAILABLE:
            self.log_message(
                "ERROR: Converter module not available. Please check installation."
            )
    ...
    def browse_files(self):
        files = filedialog.askopenfilenames(
            ...
            filetypes=[
                (
                    "All supported",
                    "*.docx;*.pdf;*.txt;*.html;*.htm;*.rtf;*.epub",
                ),
                ("All files", "*.*"),
            ],
        )
    ...
    def convert_documents(self):
        try:
            ...
            for i, file_path in enumerate(files_to_convert):
                try:
                    ...
                    self.converter.convert_file(
                        file_path, output_file, input_fmt, output_fmt
                    )

                    successful += 1
                    self.log_message(
                        f"SUCCESS: {file_path.name} -> {output_file.name}"
                    )
                ...
                self.update_status(
                    f"Converting... {i+1}/{len(files_to_convert)}"
                )
            ...
            self.update_status(
                f"Complete! {successful} converted, {failed} failed"
            )

            messagebox.showinfo(
                "Complete", f"Converted {successful} files!\nFailed: {failed}"
            )
