...
class ConverterLogger:
    ...
    def __init__(
        self, name: str = "DocumentConverter", log_level: str = "INFO"
    ):
        self.name = name
    ...
    def _setup_logger(self):
        ...
        try:
            ...
            log_file = (
                log_dir
                / f"converter_{datetime.datetime.now().strftime('%Y%m%d')}.log"
            )
    ...
    DEFAULT_CONFIG = {
        "general": {
            ...
            "default_output_directory": str(
                Path.home() / "Desktop" / "converted_documents"
            ),
    ...
    def reset_to_defaults(self, section: Optional[str] = None) -> None:
        ...
        if section:
            if section in self.DEFAULT_CONFIG:
                self.config[section] = copy.deepcopy(
        ...
        try:
            ...
            if not import_file.exists():
                raise FileNotFoundError(
                    f"Configuration file not found: {import_file}"
                )
...
class FormatDetector:
    ...
    def get_input_format_list(cls):
        ...
        for key, info in cls.SUPPORTED_INPUT_FORMATS.items():
            formats.append(
                (f"{info['name']} ({', '.join(info['extensions'])})", key)
            )
        return formats
        ...
        try:
            ...
            for item in book.get_items():
                if item.get_type() == ebooklib.ITEM_DOCUMENT:
                    ...
                    chapter_content = self._parse_html_content(
                        html_content, item.get_name()
                    )
        ...
        try:
            ...
            for element in soup.find_all(
                ...
                if text and len(text) > 3:  # Skip very short text
                    if element.name.startswith("h"):
                        level = min(
                            int(element.name[1]) + 1, 6
                        )  # Offset by 1 since book title is h1
                        content.append(("heading", level, text))
                    else:
                        content.append(("paragraph", text))
    ...
    def write(self, content, output_path):
        ...
        for item in content:
            if item[0] == "heading":
                level, text = item[1], item[2]
                lines.append(
                    f"    <h{level}>{self._escape_html(text)}</h{level}>"
                )
            elif item[0] == "paragraph":
    ...
    def write(self, content, output_path):
        ...
        for item in content:
            if item[0] == "heading":
                ...
                size = max(
                    32 - (level * 4), 20
                )  # Larger size for higher level headings
                rtf_lines.append(
                    f"\\par\\fs{size}\\b {self._escape_rtf(text)}\\b0\\fs24"
                )
            elif item[0] == "paragraph":
...
class EpubWriter(DocumentWriter):
    ...
    def write(self, content, output_path):
        ...
        try:
            ...
            for item in content:
                if item[0] == "paragraph" and item[1].lower().startswith(
                    "by:"
                ):
                ...
                if item[0] == "heading" and item[1] <= 2:
                    ...
                    if current_chapter is not None:
                        ...
                        self._finalize_chapter(
                            current_chapter, current_chapter_content
                        )
                    ...
                    current_chapter_content.append(
                        f"<h{level}>{self._escape_html(item[2])}</h{level}>"
                    )
                    ...
                    if not (
                        item[1].lower().startswith("by:")
                        and author != "Unknown Author"
                    ):
                        current_chapter_content.append(
                            f"<p>{self._escape_html(item[1])}</p>"
                        )
                    ...
                    current_chapter_content.append(
                        f"<p>{self._escape_html(item[2])}</p>"
                    )
...
class UniversalConverter:
    ...
    def __init__(
        self,
        logger_name: str = "UniversalConverter",
        ...
        if enable_caching is None:
            enable_caching = self.config_manager.get(
        ...
        self.memory_threshold_mb = self.config_manager.get(
            "performance", "memory_threshold_mb", 500
        )
        ...
        if self._should_optimize_memory():
            with self.cache_lock:
                ...
                if cache_size > 0:
                    self.cache.clear()
                    self.logger.debug(
                        f"Cleared cache ({cache_size} entries) due to high memory usage"
                    )
    ...
    def _get_cache_key(
        self, input_path: Path, output_format: str, input_format: str
    ) -> str:
    ...
    def _is_cached_valid(
        self, input_path: Path, output_path: Path, cache_key: str
    ) -> bool:
    ...
    def convert_file(
        ...
        try:
            ...
            if input_format not in self.readers:
                raise UnsupportedFormatError(
                    f"No reader available for format: {input_format}"
                )

            # Validate output format
            if output_format not in self.writers:
                raise UnsupportedFormatError(
                    f"No writer available for format: {output_format}"
                )
            ...
            if self.enable_caching:
                cache_key = self._get_cache_key(
                    input_path, output_format, input_format
                )
                if cache_key and self._is_cached_valid(
                    input_path, output_path, cache_key
                ):
            ...
            if self.enable_memory_monitoring:
                self.logger.debug(
                    f"Memory usage before conversion: {initial_memory:.1f} MB"
                )
            ...
            try:
                ...
                if self.enable_memory_monitoring:
                    ...
                    if (
                        memory_increase > 50
                    ):  # Log if memory increased by more than 50MB
                        self.logger.debug(
                            f"Memory increased by {memory_increase:.1f} MB after reading"
                        )
                ...
                raise FileProcessingError(
                    f"Failed to read {input_path}: {str(e)}"
                )
            ...
            try:
                ...
                raise FileProcessingError(
                    f"Failed to write {output_path}: {str(e)}"
                )
            ...
            if self.enable_memory_monitoring:
                ...
                if abs(total_change) > 10:  # Log significant memory changes
                    self.logger.debug(
                        f"Memory change during conversion: {total_change:+.1f} MB"
                    )

            self.logger.info(
                f"Conversion completed successfully: {input_path} -> {output_path}"
            )
    ...
    def convert_batch(
        ...
        if max_workers is None:
            max_workers = min(
                4, (os.cpu_count() or 1) + 1
            )  # Conservative default

        self.logger.info(
            f"Starting batch conversion of {len(file_list)} files with {max_workers} workers"
        )
        ...
        def convert_single_file(file_info):
            ...
            try:
                ...
                if preserve_structure and base_dir:
                    ...
                    output_ext = FormatDetector.SUPPORTED_OUTPUT_FORMATS[
                        output_format
                    ]["extension"]
                    output_file_path = output_dir / rel_path.with_suffix(
                        output_ext
                    )
                else:
                    output_ext = FormatDetector.SUPPORTED_OUTPUT_FORMATS[
                    ...
                    output_file_path = (
                        output_dir / f"{file_path.stem}{output_ext}"
                    )
                ...
                # Convert the file
                self.convert_file(
                    file_path, output_file_path, input_format, output_format
                )

                return {
                    "status": "success",
                    "file": file_path.name,
                    "output": output_file_path.name,
                    "index": index,
                }
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            # Submit all tasks
            future_to_file = {
                executor.submit(convert_single_file, (file_path, i)): (
                    file_path,
                    i,
                )
        ...
        self.logger.info(
            f"Batch conversion completed: {results['successful']} successful, "
...
class SettingsDialog:
    ...
    def create_general_tab(self):
        ...
        formats_frame = ttk.LabelFrame(
            frame, text="Default Formats", padding="10"
        )
        formats_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=(0, 15))
        formats_frame.columnconfigure(1, weight=1)

        ttk.Label(formats_frame, text="Input Format:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        input_formats = ["auto"] + [
            f[0]
            for f in FormatDetector.get_input_format_list()
            if f[0] != "auto"
        ]
        ttk.Combobox(
            formats_frame,
            textvariable=self.default_input_format,
            values=input_formats,
            state="readonly",
        ).grid(row=0, column=1, sticky=(tk.W, tk.E), pady=2)

        ttk.Label(formats_frame, text="Output Format:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10)
        )
        output_formats = [
            f[0] for f in FormatDetector.get_output_format_list()
        ]
        ttk.Combobox(
            formats_frame,
            textvariable=self.default_output_format,
        ...
        directory_frame = ttk.LabelFrame(
            frame, text="Default Directory", padding="10"
        )
        ...
        ttk.Entry(
            dir_entry_frame, textvariable=self.default_output_directory
        ...
        options_frame = ttk.LabelFrame(
            frame, text="File Handling", padding="10"
        )
        ...
        ttk.Checkbutton(
            ...
            variable=self.preserve_folder_structure,
        ).grid(row=0, column=0, sticky=tk.W, pady=2)
        ttk.Checkbutton(
            options_frame,
            text="Overwrite existing files",
            variable=self.overwrite_existing_files,
        ).grid(row=1, column=0, sticky=tk.W, pady=2)
        ttk.Checkbutton(
            options_frame,
            text="Auto-open output folder after conversion",
            variable=self.auto_open_output_folder,
    ...
    def create_performance_tab(self):
        ...
        ttk.Checkbutton(
            caching_frame,
            text="Enable intelligent caching",
            variable=self.enable_caching,
        ).grid(row=0, column=0, sticky=tk.W, pady=2)

        # Threading section
        threading_frame = ttk.LabelFrame(
            frame, text="Multi-threading", padding="10"
        )
        threading_frame.grid(
            row=1, column=0, sticky=(tk.W, tk.E), pady=(0, 15)
        )
        threading_frame.columnconfigure(1, weight=1)

        ttk.Label(threading_frame, text="Max Worker Threads:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        ttk.Spinbox(
            threading_frame,
            from_=1,
            to=16,
            textvariable=self.max_worker_threads,
            width=10,
        ).grid(row=0, column=1, sticky=tk.W, pady=2)
        ttk.Label(
            threading_frame, text=f"(CPU cores: {os.cpu_count() or 1})"
        ).grid(row=0, column=2, sticky=tk.W, padx=(10, 0))

        # Memory section
        memory_frame = ttk.LabelFrame(
            frame, text="Memory Management", padding="10"
        )
        ...
        ttk.Checkbutton(
            ...
            variable=self.enable_memory_monitoring,
        ).grid(row=0, column=0, columnspan=3, sticky=tk.W, pady=2)

        ttk.Label(memory_frame, text="Memory Threshold (MB):").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10)
        )
        ttk.Spinbox(
            memory_frame,
            ...
            to=2000,
            increment=50,
            textvariable=self.memory_threshold_mb,
            width=10,
    ...
    def create_gui_tab(self):
        ...
        ttk.Label(window_frame, text="Default Height:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10)
        )
        ttk.Spinbox(
            window_frame,
        ...
        appearance_frame = ttk.LabelFrame(
            frame, text="Appearance", padding="10"
        )
        ...
        ttk.Label(appearance_frame, text="Font Size:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10)
        )
        ttk.Spinbox(
            appearance_frame,
    ...
    def create_logging_tab(self):
        ...
        ttk.Label(level_frame, text="Log Level:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        ttk.Combobox(
            level_frame,
        ...
        output_frame = ttk.LabelFrame(
            frame, text="Output Settings", padding="10"
        )
        ...
        ttk.Label(output_frame, text="Log Retention (days):").grid(
        ...
        ttk.Spinbox(
            ...
            from_=1,
            to=365,
            textvariable=self.log_retention_days,
            width=10,
    ...
    def load_settings(self):
        ...
        self.default_input_format.set(
            general_config.get("default_input_format", "auto")
        )
        self.default_output_format.set(
            general_config.get("default_output_format", "markdown")
        )
        self.default_output_directory.set(
            general_config.get(
                "default_output_directory",
                str(Path.home() / "Desktop" / "converted_documents"),
            )
        )
        self.preserve_folder_structure.set(
            general_config.get("preserve_folder_structure", True)
        )
        self.overwrite_existing_files.set(
            general_config.get("overwrite_existing_files", False)
        )
        self.auto_open_output_folder.set(
            general_config.get("auto_open_output_folder", False)
        )
        ...
        self.max_worker_threads.set(
            performance_config.get(
                "max_worker_threads", min(4, (os.cpu_count() or 1) + 1)
            )
        )
        self.memory_threshold_mb.set(
            performance_config.get("memory_threshold_mb", 500)
        )
        self.enable_memory_monitoring.set(
            performance_config.get("enable_memory_monitoring", True)
        )
        ...
        self.show_advanced_options.set(
            gui_config.get("show_advanced_options", True)
        )
        self.remember_window_position.set(
            gui_config.get("remember_window_position", True)
        )

        # Logging settings
        logging_config = self.config_manager.get_section("logging")
        self.log_level.set(logging_config.get("log_level", "INFO"))
        self.enable_file_logging.set(
            logging_config.get("enable_file_logging", True)
        )
        self.log_retention_days.set(
            logging_config.get("log_retention_days", 30)
        )
...
class UniversalDocumentConverterGUI:
    ...
    def __init__(self, root, config_manager: Optional[ConfigManager] = None):
        ...
        self.converter = UniversalConverter(
            "GUI_Converter", config_manager=self.config_manager
        )
        ...
        self.input_format = tk.StringVar(
            value=general_config.get("default_input_format", "auto")
        )
        self.output_format = tk.StringVar(
            value=general_config.get("default_output_format", "markdown")
        )
        ...
        self.max_workers = tk.IntVar(
            value=performance_config.get(
                "max_worker_threads", min(4, (os.cpu_count() or 1) + 1)
            )
        )
        self.enable_caching = tk.BooleanVar(
            value=performance_config.get("enable_caching", True)
        )
        self.conversion_start_time = None
        self.estimated_completion_time = None

        # Set default output from config
        default_output = general_config.get(
            "default_output_directory",
            str(Path.home() / "Desktop" / "converted_documents"),
        )
    ...
    def setup_menu(self):
        ...
        edit_menu.add_command(
            label="Clear Results", command=self.clear_results
        )
        ...
        tools_menu.add_command(
            label="Open Output Folder", command=self.open_output_folder
        )
        tools_menu.add_separator()
        tools_menu.add_command(
            label="Setup File Associations...",
            command=self.setup_file_associations,
        )
        tools_menu.add_separator()
        tools_menu.add_command(
            label="Export Configuration...", command=self.export_config
        )
        tools_menu.add_command(
            label="Import Configuration...", command=self.import_config
        )
        tools_menu.add_separator()
        tools_menu.add_command(
            label="Reset to Defaults", command=self.reset_config_to_defaults
        )
    ...
    def setup_ui(self):
        ...
        format_frame = ttk.LabelFrame(
            self.main_frame, text="📄 Format Selection", padding="10"
        )
        format_frame.grid(
        ...
        self.input_format_combo = ttk.Combobox(
            ...
            values=[f[1] for f in input_formats],
            state="readonly",
        )
        self.input_format_combo.grid(
        ...
        self.output_format_combo = ttk.Combobox(
            ...
            values=[f[1] for f in output_formats],
            state="readonly",
        )
        ...
        input_frame = ttk.LabelFrame(
            self.main_frame, text="📁 Input Selection", padding="10"
        )
        input_frame.grid(
        ...
        self.input_entry = ttk.Entry(
            input_frame, textvariable=self.input_path, font=("Arial", 9)
        )
        ...
        ttk.Button(
            button_frame, text="Select Files", command=self.browse_input_files
        ...
        output_frame = ttk.LabelFrame(
            self.main_frame, text="📂 Output Location", padding="10"
        )
        output_frame.grid(
        ...
        self.output_entry = ttk.Entry(
            output_frame, textvariable=self.output_path, font=("Arial", 9)
        )
        ...
        # Options frame
        options_frame = ttk.LabelFrame(
            self.main_frame, text="⚙️ Options", padding="10"
        )
        options_frame.grid(
        ...
        ttk.Checkbutton(
            perf_frame, text="Enable caching", variable=self.enable_caching
        ).grid(row=0, column=0, sticky=tk.W)

        ttk.Label(perf_frame, text="Worker threads:").grid(
            row=0, column=1, sticky=tk.W, padx=(20, 5)
        )

        workers_spinbox = ttk.Spinbox(
            perf_frame, from_=1, to=16, width=5, textvariable=self.max_workers
        )
        ...
        button_frame.grid(
            row=6, column=0, columnspan=3, pady=(0, 15), sticky=(tk.W, tk.E)
        )
        ...
        self.results_text = tk.Text(
            text_frame, height=6, wrap=tk.WORD, font=("Consolas", 9)
        )
    ...
    def on_window_resize(self, event):
        ...
        if event.widget == self.root:
            ...
            self._resize_timer = self.root.after(
                150, self.detect_and_apply_layout
            )
    ...
    def detect_and_apply_layout(self):
        ...
        try:
            ...
            should_be_compact = (
                width < self.layout_breakpoint_width
                or height < self.layout_breakpoint_height
            )
    ...
    def on_drop(self, event):
        ...
        if files:
            ...
            if os.path.isdir(dropped_path):
                ...
                self.log_message(
                    f"📁 Folder dropped: {os.path.basename(dropped_path)}"
                )
            else:
                # Single file dropped
                self.input_path.set(dropped_path)
                self.log_message(
                    f"📄 File dropped: {os.path.basename(dropped_path)}"
                )
    ...
    def browse_input_files(self):
        ...
        files = filedialog.askopenfilenames(
            title="Select documents to convert", filetypes=filetypes
        )
        if files:
    ...
    def install_packages(self, packages):
        ...
        for package in packages:
            try:
                ...
                self.log_message(
                    f"❌ Package installation timeout for {package}"
                )
            except Exception as e:
    ...
    def restore_window_position(self):
        ...
        if not self.config_manager.get(
            "gui", "remember_window_position", True
        ):
            return
        ...
        if last_x is not None and last_y is not None:
            try:
                ...
                if (
                    0 <= last_x < screen_width - 100
                    and 0 <= last_y < screen_height - 100
                ):
    ...
    def save_current_settings(self):
        ...
        try:
            ...
            self.config_manager.set(
                "general", "default_input_format", self.input_format.get()
            )
            self.config_manager.set(
                "general", "default_output_format", self.output_format.get()
            )
            self.config_manager.set(
                "general", "default_output_directory", self.output_path.get()
            )

            # Save performance settings
            self.config_manager.set(
                "performance", "max_worker_threads", self.max_workers.get()
            )
            self.config_manager.set(
                "performance", "enable_caching", self.enable_caching.get()
            )
        ...
        if filename:
            try:
                if self.config_manager.export_config(filename):
                    messagebox.showinfo(
                        "Success", f"Configuration exported to {filename}"
                    )
                    self.log_message(
        ...
        if filename:
            try:
                if self.config_manager.import_config(filename):
                    ...
                    messagebox.showinfo(
                        "Success", f"Configuration imported from {filename}"
                    )
                    self.log_message(
    ...
    def reload_gui_settings(self):
        ...
        try:
            ...
            self.max_workers.set(
                performance_config.get(
                    "max_worker_threads", min(4, (os.cpu_count() or 1) + 1)
                )
            )
            self.enable_caching.set(
                performance_config.get("enable_caching", True)
            )
...
© 2025 Beau Lewis. All rights reserved."""
    ...
    def open_last_converted_file(self):
        ...
        if self.last_converted_file and os.path.exists(
            ...
            try:
                if os.name == "nt":  # Windows
                    ...
                    subprocess.run(
                        ["open", self.last_converted_file], check=True
                    )
                else:  # Linux and other Unix-like
                    subprocess.run(
                        ["xdg-open", self.last_converted_file], check=True
                    )
                self.log_message(
                    f"📖 Opened file: {os.path.basename(self.last_converted_file)}"
                )
            except Exception as e:
            ...
            messagebox.showwarning(
                "Warning", "No converted file to open or file no longer exists"
            )
...
After setting up file associations, you can double-click EPUB files to open them!"""
    ...
    def apply_compact_layout(self):
        ...
        width = self.root.winfo_width()
        height = self.root.winfo_height()
        compact_fonts = self.calculate_dynamic_fonts(
            width, height, compact=True
        )
    ...
    def apply_standard_layout(self):
        ...
        width = self.root.winfo_width()
        height = self.root.winfo_height()
        standard_fonts = self.calculate_dynamic_fonts(
    ...
    def adjust_widget_spacing(self, compact=False):
        ...
        if hasattr(self, "main_frame") and self.main_frame:
            for widget in self.main_frame.winfo_children():
                ...
                elif isinstance(widget, ttk.Button) and hasattr(
                    widget, "grid_info"
                ):
    ...
    def calculate_dynamic_fonts(self, width, height, compact=False):
        ...
        if compact:
            ...
            base_sizes = {
                k: max(8, int(v * 0.8)) for k, v in base_sizes.items()
            }
        else:
        ...
        for font_type, base_size in base_sizes.items():
            ...
            # Create font tuple
            font_family = (
                "Consolas" if font_type == "monospace" else "Segoe UI"
            )
    ...
    def validate_inputs(self):
        ...
        if not self.input_path.get():
            messagebox.showerror(
                "Error", "Please select input files or folder"
            )
            return False
    ...
    def convert_documents(self):
        ...
        try:
            ...
            files_to_convert = [
                f for f in files_to_convert if not f.name.startswith("~$")
            ]
            ...
            if (
                base_dir is None
                and self.preserve_structure.get()
                and len(files_to_convert) > 1
            ):
                # Find common parent directory for multiple files
                try:
                    base_dir = Path(
                        os.path.commonpath(
                            [str(f.parent) for f in files_to_convert]
                        )
                    )
                except ValueError:
                    base_dir = None
            ...
            self.log_message(
                f"⚡ Using {self.max_workers.get()} worker threads"
            )
            ...
            def progress_callback(completed, total, result):
                ...
                if completed > 0:
                    ...
                    if result["status"] == "success":
                        ...
                        self.log_message(
                            f"⏭️  Skipped (exists): {result['file']}"
                        )

                    self.update_status(
                        f"Converting... {completed}/{total} (ETA: {eta:.1f}s)"
                    )
                else:
            ...
            self.update_status(
                f"Complete! {results['successful']} files converted, {results['failed']} failed"
            )
            ...
            if results["successful"] == 1 and len(files_to_convert) == 1:
                ...
                self.root.after(
                    0, lambda: self.open_file_btn.grid()
                )  # Show the button
            else:
            ...
            completion_msg = (
                f"Successfully converted {results['successful']} files!\n"
    ...
    def dark_theme(self):
        ...
        return getattr(
            self,
            "_dark_theme",
            {"bg": "#2c3e50", "fg": "#ecf0f1", "accent": "#3498db"},
        )
...
def main():
    ...
    if (
        not config_manager.get("gui", "remember_window_position", True)
        or config_manager.get("gui", "last_window_x") is None
    ):
