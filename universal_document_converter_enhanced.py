...
class UniversalDocumentConverterEnhanced:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="Universal Document Converter with OCR",
            font=("Arial", 16, "bold"),
        )
        ...
        self.results_text.grid(
            row=10, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S)
        )
    ...
    def start_ocr_conversion(self):
        ...
        threading.Thread(
            target=self.perform_ocr_conversion, daemon=True
        ).start()
    ...
    def perform_ocr_conversion(self):
        ...
        try:
            ...
            self.update_status(
                f"OCR complete: {results['successful']} successful, {results['failed']} failed"
            )
