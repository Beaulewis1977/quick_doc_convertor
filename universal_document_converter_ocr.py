...
class DocumentConverterApp:
    ...
    def setup_logging(self):
        ...
        log_file = (
            log_dir
            / f"converter_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def create_widgets(self):
        ...
        ttk.Button(
            file_frame, text="Remove Selected", command=self.remove_selected
        ).grid(row=1, column=1, padx=(0, 5))
        ttk.Button(file_frame, text="Clear All", command=self.clear_all).grid(
            row=1, column=2
        )
        ...
        self.ocr_var = tk.BooleanVar(
            value=self.config.get("ocr_enabled", True)
        )
        ...
        self.format_var = tk.StringVar(
            value=self.config.get("output_format", "txt")
        )
        ...
        output_entry = ttk.Entry(
            settings_frame, textvariable=self.output_dir_var, width=50
        )
        ...
        ttk.Button(button_frame, text="Exit", command=self.on_closing).grid(
            row=0, column=2
        )
    ...
    def add_files(self):
        ...
        files = filedialog.askopenfilenames(
            ...
            filetypes=[
                (
                    "All supported",
                    "*.docx *.pdf *.txt *.html *.rtf *.epub *.jpg *.jpeg *.png *.tiff *.tif *.bmp *.gif *.webp",
                ),
    ...
    def process_single_file(self, file_path: str, output_dir: Path):
        ...
        use_ocr = self.config.get(
            "ocr_enabled", True
        ) and self.format_detector.is_ocr_format(str(file_path))

        if use_ocr:
            # Use OCR processing
            output_text = self.ocr_integration.process_file(str(file_path))
            output_file = (
                output_dir / f"{file_path.stem}.{self.format_var.get()}"
            )
    ...
    def convert_document(self, input_path: str, output_dir: str):
        ...
        try:
            ...
            with open(str(fallback_file), "w") as f:
                ...
                f.write(
                    "Content extraction not implemented for this format.\n"
                )
    ...
    def processing_complete(self):
        ...
        if self.processed_count == self.total_files:
            self.update_status(
                f"Completed: {self.processed_count}/{self.total_files} files processed"
            )
            messagebox.showinfo(
                "Complete",
            ...
            self.update_status(
                f"Cancelled: {self.processed_count}/{self.total_files} files processed"
            )
    ...
    def on_closing(self):
        ...
        if self.is_processing:
            if messagebox.askokcancel(
                "Quit", "Conversion is in progress. Do you want to quit?"
            ):
                self.is_processing = False
                if (
